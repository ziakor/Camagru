...
<div class="container-fluid gallerie_container">
  <nav aria-label="pagination_container" class="pagination_container">
    <ul class="pagination">
      ...
      <li class="page-item"><a class="page-link" href="<?php echo ("?page=" . ($nb + 1)) ?>"><?php echo ($nb + 1) ?></a>
      </li>
      <li class="page-item"><a class="page-link" href="<?php echo ("?page=" . ($nb + 2)) ?>"><?php echo ($nb + 2) ?></a>
      </li>
  ...
  <div class="row row_gallerie mx-auto">
    ...
    <div class="<?php echo ("col-2 no_pad div_img_gal  ") ?>" id=<?php echo "" . $key ?>>
      <img class="img_gal" src=<?php echo ("./ressources/db_images/" . $value['image_name']) . ".png" ?>
      ...
      <div class="row bar">
        ...
        <hr>
        <div class="col-4 tt">
          <a href=<?php if (array_key_exists('loggued_as', $_SESSION)) echo ("?lk=" . $value['image_name']); ?>><img
              src="./ressources/icons/like_icons.svg" alt="like_image" class="like_img icc"></a>
        </div>
        <div class="col-4 tt">
          <a href=<?php if (array_key_exists('loggued_as', $_SESSION)) echo ("?dis=" . $value['image_name']); ?>><img
              src="./ressources/icons/unlike_icons.svg" alt="like_image" class="dislike_img icc"></a>
        </div>
        <div class="col-4 tt">
          <?php
							if (array_key_exists('loggued_as', $_SESSION)) {
								if ($value['pseudo'] == $_SESSION['loggued_as'])
									?><a href=<?php echo ("?del=" . $value['image_name']) ?>><img src="./ressources/icons/delete_icons.svg"
              class="delete_icons icc"></a>
          <?php
						}
						?>
        </div>
      ...
      <div class="row">
        <div class="col-12 comment_form">
          <?php
							if (array_key_exists('loggued_as', $_SESSION)) {
								?>
          <form action="<?php echo "./src/main/comment.php?img_name=" . $value["image_name"]  ?>" id="form_comment"
            method="post">
